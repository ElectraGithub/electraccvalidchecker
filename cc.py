...
def logo():
    figlet = Figlet(font="standard").renderText("ELECTRA OP")
    return (gn + figlet)
print (logo())
print (bl + "[-] Powered by ELECTRA X GAURAV  ")
print (gn + "[+] Made By ELECTRA OP")
...
def genscard():
    ...
    return (gn + "[-] Brand : %s\n[-] Card Number : %s\n[-] Bank : %s\n[-] Name : %s\n[-] Address : %s\n[-] Country : %s\n[-] Money Range : %s\n[-] CVV : %s\n[-] Expiry : %s\n[-] Pin : %s\n============================\n[*] Telegram Id : @THE_HELL_KINGG" % (card['IssuingNetwork'] , card['CardNumber'] , card['Bank'] , card['Name'] , card['Address'] , card['Country'] , card['MoneyRange'] , card['CVV'] , card['Expiry'] , card['Pin']) + cv)
...
if opr == "1":
    ...
    print (mag + "[$] Telegram Id : @THE_HELL_KINGG" + cv)
